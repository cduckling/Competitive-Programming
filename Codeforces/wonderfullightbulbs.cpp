...
void solve() {
    ...
    vpii pts (n); for (ll i = 0; i < n; i++) cin >> pts[i].f >> pts[i].s;

    pii t;

    mii row; for (const pii i : pts) row[i.f]++; for (const pii i: row) if (i.second % 2 != 0) t.first = i.first;

    mii slp; for (const pii i : pts) slp[i.f + i.s]++; for (const pii i : slp) if (i.second % 2 != 0) t.second = i.first - t.first;
