...
void solve() {
    ...
    vvi dp (n + 1, vi (n + 1)); dp[n][0] = 1;

    for (ll i = n; i > 0; i--) for (ll j = 0; j <= n - i; j++) dp[i - 1][j] += dp[i][j], dp[i - 1][j] %= m, dp[i - 1][j + 1] += dp[i][j] * i * (n + 1 - i - j), dp[i - 1][j + 1] %= m;

    ll sum = 0; for (ll i = 0; i <= n; i++) sum += dp[0][i], sum %= m; cout << (sum + m) % m << "\n";
}
