...
void solve() {
    ...
    vi div (6); for (int i = 1; i <= n; i++) for (int j = 2; j <= 5; j++) if (a[i] % j == 0) div[j]++;

    if (div[k] || (k == 4 && div[2] >= 2)) {
        cout << 0 << "\n"; return;
    }
    ...
    if (k == 2) {
        ...
        int ans = 2; for (int i = 1; i <= n; i++) if (a[i] % 3 == 2) ans = 1; cout << ans << "\n";
    } else if (k == 4) {
        int ans = 2 - div[2]; for (int i = 1; i <= n; i++) if (a[i] % 4 == 3) ans = min(ans, 1); cout << ans << "\n";
    } else {
        int ans = 4; for (int i = 1; i <= n; i++) ans = min(ans, 5 - a[i] % 5); cout << ans << "\n";
    }
}
