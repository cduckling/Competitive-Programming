...
void solve() {
    ...
    vpii lr (n + 1); for (int i = 1; i <= n; i++) cin >> lr[i].fir >> lr[i].sec; sort(all(lr));

    vpii blx; for (int i = 1; i <= n; i++) for (int j = i + 1; j <= n; j++) if (lr[i].sec >= lr[j].fir) blx.eb(min(lr[i].fir, lr[j].fir), max(lr[i].sec, lr[j].sec)); sort(all(blx), cmp);

    int cnt = 0, last = -1;

    for (const pii i : blx) {
        if (i.fir <= last) continue;
        last = i.sec, cnt++;
    }
