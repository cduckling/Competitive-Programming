...
void solve() {
    ...
    pii p, q; cin >> p.fir >> p.sec >> q.fir >> q.sec;

    vi a (n + 1); for (ll i = 1; i <= n; i++) cin >> a[i]; sort(all(a)); ll sum = 0; for (const ll i : a) sum += i;

    const ll dist = square(p.fir - q.fir) + square(p.sec - q.sec);

    cout << (square(max(a[n] - (sum - a[n], 0ll))) <= dist && dist <= square(sum) ? "YES" : "NO") << "\n";
}
