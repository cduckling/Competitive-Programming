...
void solve() {
    ...
    vi l (n + 1); for (ll i = 1; i <= n; i++) cin >> l[i];
    vi r (n + 1); for (ll i = 1; i <= n; i++) cin >> r[i];

    ll ops = 0; for (ll i = 1; i <= n; i++) ops += max(l[i], r[i]);

    vi other (n + 1); for (ll i = 1; i <= n; i++) other[i] = min(l[i], r[i]); sort(all(other), greater<ll>());

    for (ll i = 0; i < k - 1; i++) ops += other[i];
