...
void solve() {
    ...
    vi p (n); for (int i = 0; i < n; i++) {
        for (int div = 2; div * div <= a[i]; div++) if (a[i] % div == 0) {
            p[i] = div; break;
        }
    ...
    int sum = 0; for (const int i : p) {
        if (i == 1) {
            sum ^= 1;
        } else if (i % 2 != 0) {
            sum ^= (lower_bound(g.begin(), g.end(), i) - g.begin() + 1);
        }
    }
...
int main() {
    ...
    for (int i = 2; i <= 10000000; i++) {
        ...
        g.pb(i); for (int j = i * 2; j <= 10000000; j += i) p[j] = true;
    }
