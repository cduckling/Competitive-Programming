...
void solve() {
    ...
    vi a (2 * n); for (ll i = 0; i < 2 * n; i++) cin >> a[i]; sort(all(a));

    ll l = 0, r = 0; for (ll i = 0; i < n; i++) l += a[i], l %= mod; for (ll i = n; i < 2 * n; i++) r += a[i], r %= mod;

    cout << (combo(2 * n, n) * (r - l) % mod + mod) % mod << "\n";
}

int main() {
    io

    fac[0] = 1; for (int i = 1; i <= 300000; i++) fac[i] = fac[i - 1] % mod * (i % mod) % mod;
