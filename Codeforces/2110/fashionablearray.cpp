...
void solve() {
    ...
    vi a (n); for (ll i = 0; i < n; i++) cin >> a[i]; sort(all(a));

    vb b (n); for (ll i = 0; i < n; i++) b[i] = a[i] % 2;

    vi even; for (ll i = 0; i < n; i++) if (!b[i]) even.pb(i);
    vi odd; for (ll i = 0; i < n; i++) if (b[i]) odd.pb(i);

    ll ops = inf;

    if (!even.empty()) ops = min(ops, even[0] + n - 1 - even[even.size() - 1]);
    if (!odd.empty()) ops = min(ops, odd[0] + n - 1 - odd[odd.size() - 1]);
