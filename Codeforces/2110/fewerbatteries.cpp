...
bool check(const int n, const int mid, const vi& b, const vvpii& adj) {
    vi dp (n + 1); vb vis (n + 1); vis[1] = true;

    for (int i = 1; i <= n; i++) {
        for (const pii j : adj[i]) if (dp[j.fir] >= j.sec && vis[j.fir]) dp[i] = max(dp[i], dp[j.fir]), vis[i] = true; dp[i] = min(dp[i] + b[i], mid);
    }
