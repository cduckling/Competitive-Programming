...
void solve() {
    ...
    ll neg = 0, pos = 0, ops = 0;

    for (ll i = 1; i <= n; i++) {
        if (a[i] > 0) {
            const ll OPS = min(a[i], neg); a[i] -= OPS, neg -= OPS, pos += OPS;

            if (a[i] > 0) pos += abs(a[i]), ops += abs(a[i]); a[i] = 0;
        } else if (a[i] < 0) {
            const ll OPS = min(abs(a[i]), pos); a[i] += OPS, pos -= OPS, neg += OPS;

            if (a[i] < 0) neg += abs(a[i]), ops += abs(a[i]); a[i] = 0;
        }
    }
