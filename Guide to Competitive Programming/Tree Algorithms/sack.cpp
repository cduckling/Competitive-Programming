...
void dfs(const int ver, const int par, vmii& dp, const vi& col, const vvi& adj) {
    vi c; for (const int i : adj[ver]) if (i != par) c.pb(i), dfs(i, ver, dp, col, adj);
    ...
    for (const int i : c) {
        ...
        for (const pii j : dp[i]) dp[ver][j.first] += j.second;
    }
}
...
int main() {
    ...
    vmii dp (n + 1); dfs(1, 0, dp, col, adj);
