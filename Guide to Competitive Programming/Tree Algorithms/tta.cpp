...
void dfs(const int ver, const int par, int& pos, vi& tta, vi& size, vi& id, const vvi& adj) {
    tta[pos] = ver, id[ver] = pos, size[ver] = 1;

    for (const int i : adj[ver]) if (i != par) dfs(i, ver, ++pos, tta, size, id, adj), size[ver] += size[i];
}
...
int main() {
    ...
    vi tta (n + 1), size (n + 1), id (n + 1); int pos = 1; dfs(1, 0, pos, tta, size, id, adj);
