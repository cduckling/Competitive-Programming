...
void dfs(const int ver, const int par, vb& vis, vi& p, vi& dep, vi& dp, vpii& bridge, const vvi& adj) {
    vis[ver] = true, p[ver] = par;
    ...
    for (const int i : adj[ver]) {
        ...
        if (vis[i]) {
            ...
            dep[i] = dep[ver] + 1; dfs(i, ver, vis, p, dep, dp, bridge, adj); dp[ver] += dp[i];
        }
    }
...
int main() {
    ...
    vb vis (n + 1); vi dep (n + 1), dp (n + 1), p (n + 1); vpii bridge; dfs(1, 0, vis, p, dep, dp, bridge, adj);

    for (const auto [fst, snd] : bridge) cout << fst << " " << snd << "\n";
